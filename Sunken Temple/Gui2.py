...
class Gui(Frame):
    ...
    def create_widgets(self):
        ...
        self.input_text.set("")
        entry = Entry(self.master, bg = "gray", width = 60, font=("Arial", 20), text = self.input_text)
        entry.bind("<Return>", self.key)

        self.output_text = StringVar()
        self.output_text.set("This is the sample text!\rHello World!")
        output_label = Label(self.master, bg = "gray", anchor = "nw", width = 60, height = 18, font=("Arial", 20))
        output_label.config(textvariable = self.output_text, justify = "left")

        player_stats_label = Label(self.master, bg = "gray", anchor ="nw", width = 20, height = 6, font=("Arial",20))
